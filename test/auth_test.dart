...
void main() {
  group('Auth Tests', () {
    ...
    test('login throws exception on failure', () async {
      final mockClient = MockClient((request) async {
        return http.Response(jsonEncode({'code': 'invalid_username', 'message': 'Invalid username.'}), 403);
      });

      final wp = WordPress(baseUrl: 'https://example.com', client: mockClient);
      
      expect(() => wp.auth.login('baduser', 'password'), throwsA(isA<WordPressException>()));
    });
